# Opt 1 (TX clock gating) synthesis
# Usage: dc_shell -f run_synth_opt1.tcl | tee reports/dc_opt1_run.log
...
define_design_lib WORK -path "./work_opt1"

# RTL file list for Opt 1: replace uart_transmitter/regs/top with clkgate variants
...
check_design > ${RPT_DIR}/check_design_opt1.rpt
...
puts "INFO: Compiling Opt1 (clock gating) at ${CLK_PERIOD}ns..."
...
puts "INFO: Opt1 Fmax = ${FMAX_MHZ} MHz (period = ${BEST_PERIOD} ns, WNS = ${BEST_WNS} ns)"

report_timing  -path_type full -delay_type max -max_paths 10 -significant_digits 3 \
    > ${RPT_DIR}/timing_setup_opt1.rpt
report_timing  -path_type full -delay_type min -max_paths 10 -significant_digits 3 \
    > ${RPT_DIR}/timing_hold_opt1.rpt
report_area    -hierarchy   > ${RPT_DIR}/area_opt1.rpt
report_power   -hierarchy -analysis_effort high > ${RPT_DIR}/power_dc_opt1.rpt
report_qor                  > ${RPT_DIR}/qor_opt1.rpt
report_constraint -all_violators > ${RPT_DIR}/constraint_violations_opt1.rpt

change_names -rules verilog -hierarchy

write -format verilog -hierarchy -output ${NET_DIR}/uart_top_clkgate_netlist.v
write_sdf -version 2.1 -context verilog -load_delay cell ${NET_DIR}/uart_top_clkgate.sdf
write_sdc ${NET_DIR}/uart_top_clkgate.sdc

puts "INFO: Opt1 synthesis complete. Netlist: ${NET_DIR}/uart_top_clkgate_netlist.v"
exit
