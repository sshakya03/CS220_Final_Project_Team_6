# Opt 2 (8-deep FIFO) synthesis
# Usage: dc_shell -f run_synth_opt2.tcl | tee reports/dc_opt2_run.log
...
define_design_lib WORK -path "./work_opt2"

# RTL file list for Opt 2: all opt2 variants (8-deep FIFOs)
...
check_design > ${RPT_DIR}/check_design_opt2.rpt
...
puts "INFO: Compiling Opt2 (reduced FIFO) at ${CLK_PERIOD}ns..."
...
puts "INFO: Opt2 Fmax = ${FMAX_MHZ} MHz (period = ${BEST_PERIOD} ns, WNS = ${BEST_WNS} ns)"

report_timing  -path_type full -delay_type max -max_paths 10 -significant_digits 3 \
    > ${RPT_DIR}/timing_setup_opt2.rpt
report_timing  -path_type full -delay_type min -max_paths 10 -significant_digits 3 \
    > ${RPT_DIR}/timing_hold_opt2.rpt
report_area    -hierarchy   > ${RPT_DIR}/area_opt2.rpt
report_power   -hierarchy -analysis_effort high > ${RPT_DIR}/power_dc_opt2.rpt
report_qor                  > ${RPT_DIR}/qor_opt2.rpt
report_constraint -all_violators > ${RPT_DIR}/constraint_violations_opt2.rpt

change_names -rules verilog -hierarchy

write -format verilog -hierarchy -output ${NET_DIR}/uart_top_opt2_netlist.v
write_sdf -version 2.1 -context verilog -load_delay cell ${NET_DIR}/uart_top_opt2.sdf
write_sdc ${NET_DIR}/uart_top_opt2.sdc

puts "INFO: Opt2 synthesis complete. Netlist: ${NET_DIR}/uart_top_opt2_netlist.v"
exit
