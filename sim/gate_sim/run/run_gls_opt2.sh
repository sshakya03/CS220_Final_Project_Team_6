#!/usr/bin/env bash
# Opt 2 (8-deep FIFO) gate-level simulation — VCS (school server)
# Usage: bash sim/gate_sim/run/run_gls_opt2.sh [--sdf]
...
SIM_BIN="$BIN_DIR/simv_opt2"
SDF_FLAG=""
TCHK_FLAG="+notimingchecks"
if [ "$USE_SDF" -eq 1 ]; then
    SDF_FLAG="+define+GLS"
    TCHK_FLAG="+neg_tchk"
    SIM_BIN="$BIN_DIR/simv_opt2_sdf"
...
echo "============================================"
echo " Opt 2 Gate-Level Simulation"
echo "============================================"
...
vcs -full64 \
    ...
    -o "$SIM_BIN" \
    2>&1 | tee "$LOG_DIR/compile_gls_opt2.log"

echo "Compiled: $SIM_BIN"

cd "$REPO/sim"
"$SIM_BIN" 2>&1 | tee "$LOG_DIR/sim_gls_opt2.log"

echo "Log: $LOG_DIR/sim_gls_opt2.log"
if [ "$USE_SDF" -eq 0 ]; then
    echo "VCD: $ACT_DIR/uart_opt2.vcd"
    echo ""
