#!/usr/bin/env bash
# Opt 1 (TX clock gating) gate-level simulation — VCS (school server)
# Step 1: zero-delay (no SDF) — verifies functionality
# Step 2: with SDF — generates VCD for PTPX power analysis
#
# Usage (from repo root on school server):
#   bash sim/gate_sim/run/run_gls_opt1.sh          # zero-delay
#   bash sim/gate_sim/run/run_gls_opt1.sh --sdf    # with SDF timing
...
echo "============================================"
echo " Opt 1 Gate-Level Simulation"
echo "============================================"
...
vcs -full64 \
    ...
    "$NET_DIR/uart_top_clkgate_netlist.v" \
    "$REPO/sim/uart_cs220_opt1_tb.v" \
    -o "$SIM_BIN" \
    2>&1 | tee "$LOG_DIR/compile_gls_opt1.log"

echo "Compiled: $SIM_BIN"

# Run from sim/ so ../ptpx/activity/ path in $dumpfile resolves correctly
cd "$REPO/sim"
"$SIM_BIN" 2>&1 | tee "$LOG_DIR/sim_gls_opt1.log"

echo "Log: $LOG_DIR/sim_gls_opt1.log"
if [ "$USE_SDF" -eq 0 ]; then
    echo "VCD: $ACT_DIR/uart_opt1.vcd"
    echo ""
