# Opt 3 (pipelined baud enable) PrimeTime PX
# Usage: pt_shell -f run_ptpx_opt3.tcl | tee reports/ptpx_opt3_run.log
...
read_verilog ${NET_DIR}/uart_top_pipe_netlist.v
current_design $DESIGN_NAME
link_design

read_sdc ${NET_DIR}/uart_top_pipe.sdc
read_sdf -load_delay cell ${NET_DIR}/uart_top_pipe.sdf
check_timing
update_timing -full
...
set VCD_FILE "${ACT_DIR}/uart_opt3.vcd"
if {[file exists $VCD_FILE]} {
    puts "INFO: Reading Opt3 activity from: ${VCD_FILE}"
    read_vcd -strip_path "uart_cs220_opt3_tb/dut" ${VCD_FILE}
} else {
...
report_power -hierarchy          > ${RPT_DIR}/power_avg_opt3.rpt
report_power -cell -significant_digits 4 > ${RPT_DIR}/power_cell_opt3.rpt
report_power -nosplit            > ${RPT_DIR}/power_total_opt3.rpt
report_power -nosplit

puts "INFO: Opt3 PTPX complete. Reports: ${RPT_DIR}/"
exit
